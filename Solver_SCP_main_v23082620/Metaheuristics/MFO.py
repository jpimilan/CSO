...
def iterarMFO(max_iter, iter, dimension, nsa, poblacion, bestSolutions, fitness, bestSolutionsFitness):
    #print(f"poblacion:\n{poblacion}")
    #print(f"fitness:\n{fitness}")

    # Recalcular cantidad de llamas, Eq. 3.14
    flameNo = int(np.ceil(nsa - (iter + 1) * ((nsa - 1) / max_iter)))

    mothPos = poblacion.copy() # arreglo de posiciones de las polillas
    ...
    if iter == 0:
        ...
        flames = bestSolutions[:flameNo, :]
        flamesFit = bestSolutionsFitness[:flameNo]
        ...
        flames = auxPop[:flameNo, :]
        flamesFit = auxFit[:flameNo]
    ...
    r = -1 + (iter + 1) * ((-1) / max_iter) 
    
    # preparar las llamas para mover las polillas
    temp1 = flames[:flameNo, :]                                                 # tomo las llamas activas
    temp2 = flames[flameNo - 1, :] * np.ones(shape=(nsa - flameNo, dimension))  # tomo y repito la ultima llama
